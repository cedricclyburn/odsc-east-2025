...
class AppSettings(BaseSettings):
    ...
    # Model settings
    embed_model_id: str = Field("sentence-transformers/all-MiniLM-L6-v2", env="EMBED_MODEL_ID")
    openai_model: str = Field("microsoft/phi-4", env="OPENAI_MODEL_NAME")
    openai_base_url: Optional[str] = Field(None, env="OPENAI_API_BASE")
...
class RAGPipeline:
    ...
    def _init_llm(self):
        ...
        if self.settings.openai_base_url and "phi-4" in self.settings.openai_base_url.lower():
        ...
        try:
            ...
            # Fallback to a more generic model name if using a custom endpoint
            if self.settings.openai_base_url:
                st.warning(f"Trying fallback model name 'microsoft/phi-4' with custom endpoint")
                return ChatOpenAI(
    ...
    def process_files(self, file_paths: List[str]) -> Tuple[Optional[Any], Optional[BaseRetriever], List[Document]]:
        """
    ...
    def _create_rag_chain(self, splits: List[Document]) -> Tuple[Any, BaseRetriever]:
        ...
        vectorstore = FAISS.from_documents(documents=splits, embedding=self.embedding_model)
...
class RAGPipelineFactory:
    ...
    @staticmethod
    def create_pipeline(pipeline_type: ChunkerType, settings: AppSettings, embedding_model: HuggingFaceEmbeddings) -> RAGPipeline:
...
class DocumentComparison:
    ...
    def create_token_distribution_chart(self, docling_splits: List[Document], recursive_splits: List[Document]) -> plt.Figure:
        ...
        recursive_tokens = [len(doc.page_content) // 4 for doc in recursive_splits]  # Rough estimate
...
class PerformanceMetrics:
    ...
    def display_metrics(self):
        ...
        metrics = {name: timing["duration"] for name, timing in self.timings.items() if timing["duration"] is not None}
        if metrics:
...
class PDFQAApp:
    ...
    def initialize_components(self):
        ...
        if self.file_paths:
            ...
            self.docling_chain, self.docling_retriever, self.docling_splits = self.docling_pipeline.process_files(
                self.file_paths)
            self.performance.end_timer("HybridChunker Processing")

            self.performance.start_timer(
                "RecursiveCharacterTextSplitter Processing")
            self.recursive_chain, self.recursive_retriever, self.recursive_splits = self.recursive_pipeline.process_files(
                self.file_paths)
    ...
    def _init_embeddings(self):
        ...
        return HuggingFaceEmbeddings(
            ...
            encode_kwargs={"truncate": True, "max_length": 512},
    ...
    def setup_sidebar(self):
        ...
        if uploaded_file:
            ...
            if default_file_path and os.path.exists(default_file_path):
                ...
                if pdf_files:
                    ...
                    st.sidebar.success(f"Using found PDF: {os.path.basename(selected_pdf)}")
                else:
        ...
        base_url_display = "Custom Endpoint" if self.settings.openai_base_url else "Official OpenAI API"
        ...
        if hybrid_max_tokens != self.settings.hybrid_max_tokens:
            ...
            st.sidebar.warning("Changing this parameter requires reprocessing the documents.")
            st.sidebar.button("Reprocess Documents", key="hybrid", on_click=self.initialize_components)
        ...
        if (recursive_chunk_size != self.settings.recursive_chunk_size or 
            recursive_chunk_overlap != self.settings.recursive_chunk_overlap):
            self.settings.recursive_chunk_size = recursive_chunk_size
            self.settings.recursive_chunk_overlap = recursive_chunk_overlap
            st.sidebar.warning("Changing these parameters requires reprocessing the documents.")
            st.sidebar.button("Reprocess Documents", key="recursive", on_click=self.initialize_components)
        ...
        with st.expander("Chunking Statistics", expanded=False):
            ...
            with col1:
                ...
                if self.docling_splits:
                    avg_length = sum(len(doc.page_content) for doc in self.docling_splits) / len(self.docling_splits)
                    st.metric("Avg. chunk length (chars)", f"{avg_length:.0f}")
                    
                    # Calculate token distribution if available
                    token_counts = [doc.metadata.get('token_count', 0) for doc in self.docling_splits if 'token_count' in doc.metadata]
                    if token_counts:
            ...
            with col2:
                ...
                if self.recursive_splits:
                    avg_length = sum(len(doc.page_content) for doc in self.recursive_splits) / len(self.recursive_splits)
                    st.metric("Avg. chunk length (chars)", f"{avg_length:.0f}")
                    
                    # Estimate tokens based on character count (rough estimate)
                    estimated_tokens = [len(doc.page_content) // 4 for doc in self.recursive_splits]
                    if estimated_tokens:
                        st.metric("Est. avg. tokens per chunk", f"{sum(estimated_tokens)/len(estimated_tokens):.0f}")
    ...
    def process_user_input(self, user_input):
        ...
        try:
            ...
            self.performance.start_timer("RecursiveCharacterTextSplitter Answer Generation")
            recursive_resp = self.recursive_chain.invoke({"input": user_input})
            self.performance.end_timer("RecursiveCharacterTextSplitter Answer Generation")
            ...
            with st.expander("🔍 HybridChunker Debug Information", expanded=False):
                ...
                retrieved_docs = self.docling_retriever.get_relevant_documents(user_input)
                context_text = "\n\n".join([doc.page_content for doc in retrieved_docs])
                full_prompt = self.docling_pipeline.prompt.format(context=context_text, input=user_input)
            ...
            with st.expander("🔍 RecursiveCharacterTextSplitter Debug Information", expanded=False):
                st.subheader("Full Prompt")
                retrieved_docs = self.recursive_retriever.get_relevant_documents(user_input)
                context_text = "\n\n".join([doc.page_content for doc in retrieved_docs])
                full_prompt = self.recursive_pipeline.prompt.format(context=context_text, input=user_input)
                st.code(full_prompt, language="markdown")
            ...
            with st.expander("Retrieved Documents Comparison", expanded=False):
                ...
                if docling_docs and recursive_docs:
                    ...
                    docling_similarities = self.document_comparison.compute_similarity(user_input, docling_docs)
                    recursive_similarities = self.document_comparison.compute_similarity(user_input, recursive_docs)
                    
                    # Create a bar chart comparing similarities
                    fig, ax = plt.subplots(figsize=(10, 5))
                    x = np.arange(min(len(docling_similarities), len(recursive_similarities)))
                    width = 0.35
                    
                    if x.size > 0:  # Check if there are documents to compare
                        rects1 = ax.bar(x - width/2, docling_similarities[:len(x)], width, label='HybridChunker', color='green', alpha=0.7)
                        rects2 = ax.bar(x + width/2, recursive_similarities[:len(x)], width, label='RecursiveCharacterTextSplitter', color='red', alpha=0.7)
...
- Avg. chunk length (chars): {sum(len(doc.page_content) for doc in self.recursive_splits) / len(self.recursive_splits):.0f}
            ...
            for doc in self.docling_splits:
                ...
                token_count = doc.metadata.get('token_count', char_count // 4)  # Use actual or estimate
                csv_buffer.write(f"HybridChunker,{chunk_id},{page},{char_count},{token_count}\n")
            
            for doc in self.recursive_splits:
                chunk_id = "N/A"  # RecursiveCharacterTextSplitter doesn't assign chunk IDs
                page = doc.metadata.get('page', 'N/A')
                char_count = len(doc.page_content)
                token_count = char_count // 4  # Estimate
                csv_buffer.write(f"RecursiveCharacterTextSplitter,{chunk_id},{page},{char_count},{token_count}\n")
    ...
    def run(self):
        ...
        if user_input := st.chat_input("Ask anything about the PDF..."):
            ...
            if not self.docling_retriever or not self.recursive_retriever:
                st.error("RAG components failed to initialize. Please check your configuration.")
                return
