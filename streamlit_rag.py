    ...
    try:
        ...
        for i, doc in enumerate(docs):
            with st.expander(f"Document {i+1}"):
                # Use border styling instead of background color for better legibility
                st.markdown(
                    f"""
...
def main():
    ...
    # st.sidebar.info(f"Using LLM: {GEN_MODEL_ID}")
    ...
    if user_input := st.chat_input("Ask anything about the PDF..."):
        ...
        # Show side-by-side document retrieval results
        st.header("Retrieved Documents Comparison")
        
        # Apply custom CSS to make the content wider
        st.markdown("""
        <style>
        ...
        # Create a wider container with custom HTML/CSS
        ...
        try:
            ...
            # Get retrieved documents directly from the retriever for debugging
            with st.expander("🔍 Debug: HybridChunker Full Prompt"):
                ...
                for i, doc in enumerate(retrieved_docs):
                    ...
                    if 'chunk_id' in doc.metadata:
                        st.markdown(f"**Chunk ID**: {doc.metadata['chunk_id']}")
                    st.markdown("**Content**:")
                    st.markdown(f"```\n{doc.page_content}\n```")
        except Exception as e:
        ...
        try:
            ...
            # Get retrieved documents directly from the retriever for debugging
            with st.expander("🔍 Debug: RecursiveCharacterTextSplitter Full Prompt"):
        ...
        # Show answers sequentially (instead of side-by-side comparison)
